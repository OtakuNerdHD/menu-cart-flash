...
-- Ajuste na ordem de resolução de escopo para evitar vazamento de GUC entre conexões
-- Prioriza header da requisição e/ou escopo da sessão mantida, antes de GUC

-- app.start_session: primeiro header, depois GUC, por fim variável legacy
...
DECLARE
  ...
  v_team text := COALESCE(v_team_header, v_team_guc, v_team_legacy); -- NULL no master
  v_new_id uuid;
...
BEGIN
  ...
  RAISE NOTICE 'start_session: uid=%, team_header=%, team_guc=%, team_legacy=%, chosen_team=% role=%',
    v_uid, COALESCE(v_team_header,'<NULL>'), COALESCE(v_team_guc,'<NULL>'), COALESCE(v_team_legacy,'<NULL>'), COALESCE(v_team,'<NULL>'), p_role_at_login;
...
-- app.end_other_sessions_current_scope: primeiro escopo da sessão mantida, depois header, depois GUC
...
BEGIN
  ...
  v_scope := COALESCE(v_team_from_keep, v_team_header, v_team_guc, v_team_legacy);

  RAISE NOTICE 'end_other_sessions_current_scope: uid=%, team_keep=%, team_header=%, team_guc=%, team_legacy=%, chosen_scope=% keep=%',
    v_uid, COALESCE(v_team_from_keep,'<NULL>'), COALESCE(v_team_header,'<NULL>'), COALESCE(v_team_guc,'<NULL>'), COALESCE(v_team_legacy,'<NULL>'), COALESCE(v_scope,'<NULL>'), p_keep_session;
